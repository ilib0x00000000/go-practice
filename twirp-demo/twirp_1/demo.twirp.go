...
func (c *helloWorldProtobufClient) Hello(ctx context.Context, in *HelloReq) (*HelloResp, error) {
	ctx = ctxsetters.WithPackageName(ctx, "twirp_demo.HelloWorld")
...
const HelloWorldPathPrefix = "/twirp/twirp_demo.HelloWorld.HelloWorld/"
...
func (s *helloWorldServer) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	...
	switch req.URL.Path {
	case "/twirp/twirp_demo.HelloWorld.HelloWorld/Hello":
...
func (s *helloWorldServer) serveHelloForm(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	var err error
	ctx = ctxsetters.WithMethodName(ctx, "Hello")
	ctx, err = callRequestRouted(ctx, s.hooks)
	if err != nil {
	...
	if v, ok := req.Form["subject"]; ok {
		reqContent.Subject = v[0]
	}
